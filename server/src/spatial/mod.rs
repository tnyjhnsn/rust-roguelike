...
pub fn move_entity(entity: Entity, moving_from: usize, moving_to: usize) {
    ...
    lock.tile_content[moving_from].iter()
        .for_each(|(_, blocks)| if *blocks { from_blocked = true; } );
    lock.tile_content[moving_to].iter()
        .for_each(|(_, blocks)| if *blocks { to_blocked = true; } );
    lock.blocked[moving_from].1 = from_blocked;
    lock.blocked[moving_to].1 = to_blocked;
}

pub fn for_each_tile_content_with_gamemode<F>(idx: usize, mut f: F) -> GameState
where F : FnMut(Entity)->Option<GameState>
{
    let lock = SPATIAL_MAP.lock().unwrap();
    for entity in lock.tile_content[idx].iter() {
